/*  UDP Client program (udpclient.c)
 *
 *  本程式啟動後向 Server (udpserver) 要求連線，
...
/* bind function is optional, so Nelson mark here to test. It is still workable.
...
*/
    ...
    if (sendto(fd, data, ndata, 0, 
                (struct sockaddr*)&servaddr, size) == -1) {
